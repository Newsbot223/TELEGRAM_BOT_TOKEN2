...
module.exports = {
  ...
  from: {
    ...
    /* TEMPORARY: pinned to the exact mailbox reservation.js already
       sends from successfully, to isolate whether order emails were
       failing because of the sender address/domain verification
       specifically. Revert to a dedicated orders@ mailbox (or restore
       the EMAIL_FROM_ADDRESS env var here) once order emails are
       confirmed working. */
    address: process.env.EMAIL_FROM_ADDRESS || 'reservierung@takashi-restaurant.com'
  },
  ...
  restaurant: {
    ...
    /* Same links already used in reservation.js's footer / the
       website's own review section — reused here, not new
       destinations. "Order again" has no self-service reorder flow
       yet, so its button stays a styled placeholder (href="#") per
       the brief, ready to wire up whenever that flow exists. */
    website:        'https://takashi-restaurant.com',
    instagramUrl:   'https://www.instagram.com/takashi_restaurant_reutlingen_',
    googleReviewUrl: 'https://www.google.com/maps/place/TAKASHI+RESTAURANT/@48.4901822,9.2151207,21z'
  },
